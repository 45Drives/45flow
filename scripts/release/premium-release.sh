...
EXAMPLES:
  # Bump patch version and build all platforms for free version
  bash scripts/release/release.sh --bump patch --os all
  
  # Build premium version for mac only (no version bump)
...
main() {
  ...
  if [[ ! -f "$CONFIG_FILE" ]]; then
    ...
    echo "Falling back to default .env.orchestrator"
